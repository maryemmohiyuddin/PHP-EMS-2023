...
if (isset($_SESSION["user_email"])) {
    ...
    if ($result && $result->num_rows > 0) {
        ...
        // user data not found or error in query
    ...
    if ($_SERVER["REQUEST_METHOD"] === "POST") {
        ...
        $city = $_POST["city"];
// Validate user name
if ($user_name != $row["name"]) { // Check if the name has changed
    ...
    $stmt_check_name->bind_param("ss", $user_name, $user_email);
    ...
    if ($result_check_name && $result_check_name->num_rows > 0) {
        ...
        echo "<script>setTimeout(function(){ window.location = 'useredit.php'; });</script>"; // Redirect after 3 seconds
        exit();
    }
    ...
    $stmt_check_name->close();
        ...
        if (isset($_FILES["picture"]) && $_FILES["picture"]["error"] == 0) {
            ...
            if (!in_array($extension, $allowed_extensions)) {
                ...
                echo "<script>setTimeout(function(){ window.location = 'useredit.php'; }, 3000);</script>"; // Redirect after 3 seconds
                exit();
            }
        ...
        $_SESSION["user_picture"] = $new_profile_picture_path;
        ...
        $stmt_update->bind_param("sssss", $user_name, $gender, $city, $new_profile_picture_path, $user_email);
        $stmt_update->execute();
        if (!$stmt_update) {
