...
<?php
include('connection.php');
// Fetch events from the database (replace with your database query)
$query = "SELECT event_name AS title, start_date AS start, end_date AS end FROM events";
$result = mysqli_query($conn, $query);

$events = array();

while ($row = mysqli_fetch_assoc($result)) {
    // Format the start and end dates to exclude the time
    $row['start'] = date('Y-m-d', strtotime($row['start']));
    $row['end'] = date('Y-m-d', strtotime($row['end']));
                ...
                <div class="ps-3">
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb mb-0 p-0">
                            ...
                            <li class="" aria-current="page">Event Dates</li>
                        </ol>
                    </nav>
                </div>
